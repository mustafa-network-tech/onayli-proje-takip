...
export default async function Detail({params,searchParams}:{params:Promise<{type:string,id:string}>,searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const visibleBuildings=p.buildings.filter(b=>statusMatch(manufacturing(b,type).percent,q.status,b.isCancelled));
 ...
 const notStarted=p.buildings.filter(b=>!b.isCancelled).length-completed-ongoing;
 const totalHp=p.buildings.reduce((a,b)=>a+(b.isCancelled?0:b.bbkHp),0);
 ...
 return <>
  ...
  <div className="grid">{[["Bina (Tamamlanan / Toplam)",`${completed} / ${p.buildings.length}`],["Toplam HP",hpDisplay],["Tamamlanan",completed],["Devam Eden",ongoing],["Başlanmayan",notStarted],["İptal",p.buildings.filter(b=>b.isCancelled).length],["HP Bazlı İlerleme",`%${formatProgress(progress)}`]].map(([x,n])=><div className="card kpi" key={x}><span className="muted">{x}</span><strong>{n}</strong></div>)}</div>
  <section className="section"><h2>Binalar</h2><form className="filters"><select name="status" defaultValue={q.status??""}><option value="">Tümü</option><option value="incomplete">Tamamlanmayan</option><option value="completed">Tamamlanan</option><option value="cancelled">İptal</option></select><button>Filtrele</button></form><div className="table-wrap"><table>
