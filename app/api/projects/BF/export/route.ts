...
type BuildingRow={projectId:string;central:string;district:string;neighborhood:string;street:string;doorNumber:string;uavt:string;hp:number;pstn:number;dsl:number;cable:string;splice:string;obk:string;status:string;progress:string;description:string;completed:boolean;cancelled:boolean};
...
function excelResponse(rows:BuildingRow[],baseName:string){
 ...
 const completedStyle={fill:{patternType:"solid",fgColor:{rgb:"C6EFCE"}},font:{color:{rgb:"17212B"}}},totalStyle={fill:{patternType:"solid",fgColor:{rgb:"D9EAF7"}},font:{bold:true,color:{rgb:"17212B"}}};
 rows.forEach((b,index)=>{if(b.cancelled)styleRow(sheet,index+1,{fill:{patternType:"solid",fgColor:{rgb:"FFC7CE"}},font:{color:{rgb:"9C0006"}}});else if(b.completed)styleRow(sheet,index+1,completedStyle)});styleRow(sheet,rows.length+1,totalStyle);XLSX.utils.book_append_sheet(book,sheet,"BF Bina Listesi");
...
function totalHp(rows:BuildingRow[]){return total(rows,"hp")}
