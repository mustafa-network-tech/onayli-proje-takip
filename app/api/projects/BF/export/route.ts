...
function excelResponse(rows:BuildingRow[],baseName:string){
 ...
 rows.forEach((b,index)=>{if(b.completed)styleRow(sheet,index+1,completedStyle)});styleRow(sheet,rows.length+1,totalStyle);XLSX.utils.book_append_sheet(book,sheet,"BF Bina Listesi");
...
async function pdfResponse(rows:BuildingRow[],district:string|undefined,baseName:string){
 ...
 const draw=(values:(string|number)[],options:{header?:boolean;completedStyle?:boolean;total?:boolean}={})=>{const h=options.header?42:52;if(doc.y+h>doc.page.height-18){doc.addPage();header();draw(columns,{header:true})}const y=doc.y;if(options.completedStyle)doc.save().fillColor("#C6EFCE").rect(18,y,tableWidth,h).fill().restore();if(options.total)doc.save().fillColor("#D9EAF7").rect(18,y,tableWidth,h).fill().restore();doc.save().lineWidth(1).strokeColor("#52606D").rect(18,y,tableWidth,h).stroke();let gx=18;for(const w of widths.slice(0,-1)){gx+=w;doc.moveTo(gx,y).lineTo(gx,y+h)}doc.stroke().restore();let x=18;doc.fillColor("#17212B").fontSize(options.header?6.6:7);values.forEach((v,i)=>{doc.text(String(v??""),x+2,y+4,{width:widths[i]-4,height:h-8,ellipsis:true,lineGap:1});x+=widths[i]});doc.y=y+h};
 draw(columns,{header:true});rows.forEach(r=>draw(Object.values(toRecord(r)),{completedStyle:r.completed}));draw([`TOPLAM ${rows.filter(r=>r.completed).length} / ${rows.length}`,"","","","","","",totalHp(rows),""],{total:true});doc.end();
 return fileResponse(await done,`${baseName}.pdf`,"application/pdf");
}

function toRecord(b:BuildingRow){return {"Proje ID":b.projectId,"Proje":"BF","Santral":b.central,"İlçe":b.district,"Mahalle":b.neighborhood,"Cadde / Sokak":b.street,"Bina No":b.doorNumber,"HP":b.hp,"Açıklama":b.description}}
