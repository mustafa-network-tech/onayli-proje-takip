...
async function pdfResponse(rows:BuildingRow[],district:string|undefined,baseName:string){
 ...
 draw(columns,{header:true});rows.forEach(r=>draw(Object.values(toRecord(r)),{yellow:r.completed}));draw([`TOPLAM ${rows.filter(r=>r.completed).length} / ${rows.length}`,"","","","","","",totalHp(rows),""],{total:true});doc.end();return fileResponse(await done,`${baseName}.pdf`,"application/pdf");
}
