  ...
  return <div className="tafics-module">
    ...
    <div className="table-wrap" tabIndex={0} role="region" aria-label="TAFICS proje tablosu, tüm sütunlar için yatay kaydırın">
      <table><thead><tr>{taficsHeaders.map(label => <th scope="col" key={label}>{label}</th>)}<th scope="col">İşlemler</th></tr></thead>
        <tbody>{visible.map((row, index) => <tr key={row.id} className={row.completionStatus === "TAMAMLANDI" ? "completed-row" : undefined}>
          ...
          <td className="tafics-description"><DescriptionEditor value={row.description} endpoint={`/api/tafics/${encodeURIComponent(row.id)}/description`} maxLength={30000} disabled={busy || editing !== undefined} onSaved={description => setRecords(previous => previous.map(item => item.id === row.id ? { ...item, description } : item))} /></td>
