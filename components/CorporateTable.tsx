...
import { CorporateEdit, CorporateProgress } from "./CorporateActions";
...
export default function CorporateTable({ projects, initialFilters }: { projects: CorporateProjectRow[]; initialFilters: Filters }) {
  ...
  return <div className="corporate-module">
    <div className="top"><div><h1>Kurumsal Projeler</h1><p className="muted">TTVPN Projeleri · Kablo ve Ek yapıldığında tamamlanır</p></div><Link className="button" href="/corporate/import">Kurumsal Excel Yükle</Link></div>
    ...
    <form className="filters section" onSubmit={event => { event.preventDefault(); const data = new FormData(event.currentTarget); setFilters({district:String(data.get("district") ?? ""),q:String(data.get("q") ?? ""),status:String(data.get("status")) as CorporateStatus}); setSelectionOnly(false); }}>
